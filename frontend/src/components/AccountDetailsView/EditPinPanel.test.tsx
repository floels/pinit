...
const testPin: PinWithFullDetails = {
...
it("can submit title and description change via PATCH, then calls onSave callback", async () => {
  ...
  fetchMock.mockOnceIf(
    updatePinUrl,
    JSON.stringify({
      unique_id: testPin.id,
      image_url: testPin.imageURL,
      title: "Updated title",
...
it("shows error message on failed PATCH", async () => {
  ...
  fetchMock.mockOnceIf(updatePinUrl, "{}", { status: 500 });
...
it("calls onDelete and onClose after successful DELETE", async () => {
  ...
  fetchMock.mockOnceIf(updatePinUrl, "", { status: 204 });
...
it("shows error message on failed DELETE", async () => {
  ...
  fetchMock.mockOnceIf(updatePinUrl, "{}", { status: 403 });
...
it("sends PATCH request with auth token", async () => {
  ...
  fetchMock.mockOnceIf(updatePinUrl, JSON.stringify({
    unique_id: testPin.id,
    image_url: testPin.imageURL,
    title: testPin.title,
