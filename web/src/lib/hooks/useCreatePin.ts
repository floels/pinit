...
export const useCreatePin = () => {
  ...
  return useMutation({
    mutationFn: async ({ pinImageFile, pinDetails }: CreatePinVariables) => {
      ...
      const presignedUrlResponse = await fetchAuthenticated(
      ...
      const s3Response = await fetchExternal(upload_url, {
        method: "PUT",
      ...
      const createPinResponse = await fetchAuthenticated(API_URL_CREATE_PIN, {
        method: "POST",
